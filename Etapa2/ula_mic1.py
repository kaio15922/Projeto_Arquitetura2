...
def executar_simulador():
    arq_regs = input("Arquivo de Registradores Iniciais (Etapa 2): ").strip()
    arq_etapa2 = input("Arquivo de Instruções 21 bits (Etapa 2): ").strip()

    if os.path.exists(arq_etapa2) and os.path.exists(arq_regs):
        with open(arq_etapa2, "r") as ent, open("log_etapa2.txt", "w") as log:
            ...
            for linha in ent:
                ir = linha.strip()
                if not ir or len(ir) != 21: 
                    continue

                # Decodificação do Instruction Register (IR) em sinais de controle
                controle_ula = ir[0:8]
                controle_c = ir[8:17]
                controle_b_str = ir[17:21]

                log.write(f"Cycle {pc_count}\n")
                log.write(f"ir = {controle_ula} {controle_c} {controle_b_str}\n\n")
