...
def traduzir_instrucao(arquivo_entrada, arquivo_saida):
    with open(arquivo_entrada, "r") as entrada, open(arquivo_saida, "w") as saida:
        
        for linha in entrada:
            linha = linha.strip()

            # Verifica se a linha é do tipo ILOAD x
            if linha.startswith("ILOAD"):
                partes = linha.split()
...
def executar_simulador():
    ...
    arq_instrucoes = "instrucoes.txt".strip()
    arq_dados3 = "dados_entregavel.txt".strip()
    arq_etapa3 = "microinstrucoes.txt".strip()

    traduzir_instrucao(arq_instrucoes,arq_etapa3)

    if os.path.exists(arq_etapa3) and os.path.exists(arq_regs):
        with open(arq_etapa3, "r") as ent, open("log_entregavel.txt", "w") as log:
            ...
            for linha in ent:
                ...
                # CASO ESPECIAL: BIPUSH (READ=1 e WRITE=1)
                if read == 1 and write == 1:
                    valor_bipush = int(controle_ula, 2) 
                    MBR = valor_bipush 
                    H = MBR
